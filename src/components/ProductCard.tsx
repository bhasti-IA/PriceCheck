...
export function ProductCard({ product, allSupermarkets, userCoords, onClick }: ProductCardProps) {
  ...
  // Map prices by supermarket for consistent display order
  ...
  return (
    <div
      onClick={onClick}
      className={`group flex flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition-all duration-300 hover:-translate-y-0.5 hover:shadow-xl ${onClick ? 'cursor-pointer' : ''}`}
    >
      {/* Product header */}
      <div className="flex items-start gap-3 p-4 sm:gap-4 sm:p-5">
        <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-slate-50 to-slate-100 text-2xl sm:h-16 sm:w-16 sm:text-3xl">
          {product.image_url || '📦'}
        </div>
      ...
      {/* Savings badge */}
      {savings > 0 && (
        <div className="mx-4 mb-3 flex items-center gap-2 rounded-lg bg-gradient-to-r from-emerald-50 to-teal-50 px-3 py-2 sm:mx-5">
      ...
      <div className="flex-1 space-y-1.5 px-4 pb-4 sm:px-5">
        {allSupermarkets.map((sm) => {
          ...
          if (!priceEntry) return (
            <div
              key={sm.id}
              className="flex items-center justify-between rounded-lg px-3 py-2 opacity-40"
            >
              ...
              <div className="flex shrink-0 items-center gap-2 sm:gap-3">
                ...
                <span
                  className={`text-sm font-semibold ${
                    isLowest ? 'text-emerald-700' : 'text-slate-700'
                  }`}
                >
